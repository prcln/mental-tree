...
export const AuthProvider = ({ children }) => {
  ...
  // Wrap handleAutoSpawn in useCallback to stabilize it
  const handleAutoSpawn = useCallback(async (userId) => {
    try {
      ...
      if (result.spawned > 0) {
        setFruitSpawnTrigger(prev => prev + 1);
      }
  ...
  // Wrap analytics functions in useCallback
  const startAnalyticsSession = useCallback(async (userId) => {
    try {
  ...
  }, [sessionChecked, startAnalyticsSession, handleAutoSpawn]);
  ...
  }, [startAnalyticsSession, handleAutoSpawn, endAnalyticsSession]);
  ...
  }, [user, startAnalyticsSession, endAnalyticsSession]);
